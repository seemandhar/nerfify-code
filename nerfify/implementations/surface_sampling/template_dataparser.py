...
@dataclass
class BlenderWithDepthDataParserConfig(DataParserConfig):
    """Blender dataset parser config"""

    _target: Type = field(default_factory=lambda: BlenderWithDepth)
...
@dataclass
class BlenderWithDepth(DataParser):
    """Blender Dataset
    ...
    def _generate_dataparser_outputs(self, split="train"):
        ...
        depth_file_names = []
        for frame in meta["frames"]:
            depth_file_name = self.data / Path(frame["depth_file_name"].replace("./", "") + ".png")
            depth_file_names.append(depth_file_name)
        ...
        metadata = {"depth_files" : depth_file_names}
        print(metadata)
        exit(0)
