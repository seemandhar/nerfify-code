...
import os
import imageio
from ..all_utils import img_utils
...
class Evaluator:
    ...
    def evaluate(self, output, batch, output_path):
        ...
        for i in range(cfg.enerf.cas_config.num):
            ...
            # for i, pred_img in enumerate(pred_rgb):
                # cv2.imwrite(f"pred_{i}.png", pred_img * 255)
            # exit(0)
            masks = (batch[f'msk_{i}'])
            # print(masks.shape, B, h, w)
            # print(pred_rgb.shape, gt_rgb.shape)
            # exit(0)
            masks = (masks.reshape(B, h, w).cpu().numpy() >= 1).astype(np.uint8)
            ...
            for b in range(B):
                ...
                if output_path is not None:
                    ...
                    img_path = os.path.join('{}_{}_{}.png'.format(batch['meta']['scene'][b], batch['meta']['tar_view'][b].item(), batch['meta']['frame_id'][b].item()))
                    imageio.imwrite(str(output_path) + "/" + img_path, (img*255.).astype(np.uint8))
                    print(img_path)
    ...
    def summarize(self, file_to_write=None):
        ret = {}
