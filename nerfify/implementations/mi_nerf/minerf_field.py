...
class MiMLP(nn.Module):
    def __init__(self, input_enc_ch, hidden, out_ch, num_layers, activation=F.relu):
        ...
        self.out_ch = out_ch
        # linear layers but each layer receives concatenation of current feature and encoded inputs
        self.linears = nn.ModuleList()
        # first layer consumes input encoding only
        self.linears.append(nn.Linear(input_enc_ch, hidden))
        for i in range(1, num_layers - 1):
            self.linears.append(nn.Linear(hidden + input_enc_ch, hidden))
        self.linears.append(nn.Linear(hidden + input_enc_ch, out_ch))
    ...
    def forward(self, enc_input: torch.Tensor) -> torch.Tensor:
        ...
        for i, lin in enumerate(self.linears):
            if i == 0:
                ...
                h = lin(torch.cat([h, enc_input], dim=-1))
...
class MiNeRFField(Field):
    ...
    def __init__(
        ...
        # )
        self.mlp_base = MiMLP(
            input_enc_ch=self.position_encoding.get_out_dim(),
            hidden=base_mlp_layer_width,
            num_layers=base_mlp_num_layers,
            out_ch = head_mlp_layer_width,
            activation=nn.ReLU()
        )
